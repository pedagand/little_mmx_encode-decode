...
Proof.
  ...
  rewrite H12.
  assert (length zero_8_bits = 8) by auto.
  assert (get_first_n_bit  (zero_8_bits) 8 = (zero_8_bits,[])) by (apply get_first_n_bit_size_nil_n; auto).
  rewrite H14.
  rewrite ret_rewrite. 
  apply commut_equal in H2.
  ...
  rewrite H2.
  apply commut_equal in H3.
  apply operand_to_bin_hypothesis2_d_n in H3.
  rewrite H3.
  apply commut_equal in H4.
  apply operand_to_bin_hypothesis3_d_n in H4.
  rewrite H4.  
  assert ({|
       instr_opcode_t_i := instr_opcode_t_i i;
       instr_operande1_t_i := instr_operande1_t_i i;
